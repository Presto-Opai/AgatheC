# ⚡ SUPER AGATHE

Un petit jeu fait avec 💛 pour la plus forte.

**12 mini-jeux qui s'enchaînent au hasard, de plus en plus vite.**
Chaque jeu est tout simple — mais la vitesse monte à chaque manche. 3 cœurs. Bonne chance.

## Les mini-jeux

| | | |
|---|---|---|
| 💥 Détruis tout ! | 🐈 Caresse le chat | 🎤 Chante juste ! |
| 🚴 Pédale plus vite ! | 🤿 Nage plus profond ! | 🐟 Caresse le poisson |
| 🦁 Combat la manticore ! | 💃 Danse danse danse ! | 🥞 Fais sauter la crêpe ! |
| 🚫 Bloque son numéro ! | 🗣️ Crie un bon coup ! | 🍦 Miam la glace ! |

## Jouer

- **Mobile** : tout se joue au doigt (tape, frotte, glisse). Mets le son 🔊 — le chat ronronne vraiment.
- **Laptop** : souris + clavier (flèches pour danser/pédaler/chanter, espace pour crier/plonger/faire sauter la crêpe).

## Héberger

C'est un seul fichier, `index.html`, zéro dépendance.
Sur GitHub : **Settings → Pages → Deploy from a branch → branche choisie, dossier `/` (root)** — et c'est en ligne.
